...
import           Data.Aeson (FromJSON, Value (Array, Object), genericParseJSON, withObject)
...
instance FromJSON LauncherOptions where
    parseJSON = withObject "LauncherOptions" $ \o ->
        genericParseJSON defaultOptions $ Object $
            -- This allows not specifying these 3 fields in the
            -- configuration file at all. @<>@ for hashmaps is
            -- left-biased, so it only adds new keys if there
            -- are aren't any yet.
            o <> HM.fromList
